...
class RankingsService {
  ...
  public async createNewRanking(
    ...
    if (isEmptyObject(rankingObject))
      throw new HttpException(400, "RankingObject empty");

    const current: WeeklyRanking[] = await this.getByLeagueAndTime(
      rankingObject.leagueId,
      rankingObject.week,
      rankingObject.year
    );
    if (!isEmptyObject(current))
      throw new HttpException(400, "RankingObject already exists");
  ...
  public async getRankingById(rankingId: string): Promise<WeeklyRanking> {
    const rankingResult: WeeklyRanking = await this.weeklyRankings.findById(
      rankingId
    );

    if (isEmptyObject(rankingResult))
      throw new HttpException(400, "RankingId did not exist");
  ...
  public async getByLeagueId(leagueId: string): Promise<WeeklyRanking[]> {
    ...
    const rankingResults: WeeklyRanking[] = await this.weeklyRankings
      .find(query)
      .sort({ year: 1, week: 1 });
  ...
  public async getByLeagueAndTime(
    leagueId: string,
    week: number,
    year: number
  ): Promise<WeeklyRanking[]> {
